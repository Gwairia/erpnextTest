var Rollup = (function (exports) {
	...
	class RowManager {
	    ...
	    expandAllNodes() {
	        ...
	        rootNodes.map(function (row) {
	            this$1.openSingleNode(row.meta.rowIndex);
	        });
	    }
	    ...
	    collapseAllNodes() {
	        ...
	        rootNodes.map(function (row) {
	            this$1.closeSingleNode(row.meta.rowIndex);
	        });
	    }
	...
	frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
		...
		get_data_for_print() {
			...
			var indices = this.datatable.datamanager.getFilteredRowIndices();
			var rows = indices.map(function (i) { return this$1.data[i]; });
							...
							fields: [
								{
									...
									change: function () {
										...
										frappe.model.with_doctype(doctype, function () {
											var fields = frappe.meta.get_docfields(doctype)
	...
	frappe.templates['print_grid'] = ' {% if title %} <h2>{{ __(title) }}</h2> <hr> {% endif %} {% if subtitle %} {{ subtitle }} <hr> {% endif %} <table class="table table-bordered">          <thead>         <tr>         {% for col in columns %}             {% if col.name && col._id !== "_check" %}             <th style="min-width: {{ col.minWidth }}px"                 {% if col.docfield && frappe.model.is_numeric_field(col.docfield) %}                     class="text-right"                 {% endif %}>{{ __(col.name) }}</th>             {% endif %}         {% endfor %}         </tr>     </thead>          <tbody>         {% for row in data %}             <tr>             {% for col in columns %}                 {% if col.name && col._id !== "_check" %}                      {% var value = col.fieldname ? row[col.fieldname] : row[col.id]; %}                      <td>                         {{                             col.formatter                                 ? col.formatter(row._index, col._index, value, col, row, true)                                 : col.format                                     ? col.format(value, row, col, data)                                     : col.docfield                                         ? frappe.format(value, col.docfield)                                         : value                         }}                     </td>                 {% endif %}             {% endfor %}             </tr>         {% endfor %}     </tbody> </table>  ';
	...
	frappe.ui.GroupBy = class {
		...
		setup_group_by_area() {
			...
			var sql_aggregate_function = [{name:'count', label: 'Count'}, {name:'sum', label: 'Sum'}, {name:'avg', label:'Average'}];
			...
			$('.aggregate-function').on('change', function () {
				this$1.report_view.meta.fields.forEach(function (field) {
					var fn = $('.aggregate-function option:selected').val();
					if(fn === 'sum' || fn === 'avg') {
		...
		apply_group_by() {
			...
			if(this.aggregate_function === 'count') {
				var group_by_query = 'count(' + this.report_view.field_type + '.'+ this.group_by+') as ' + this.group_by + '_Count';
			...
			var remove_columns = this.report_view.columns.filter(function (column) { return ![this$1.aggregate_on, this$1.group_by].includes(column.field); });
			remove_columns.forEach(function (col) {
				this$1.report_view.remove_column_from_datatable(col);
			});
		...
		remove_group_by() {
			...
			this.current_cols.forEach(function (col, i) {
				this$1.report_view.add_column_to_datatable(col.field, this$1.doctype, i);
			});
